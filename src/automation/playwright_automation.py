...
class PlaywrightAutomation:
    ...
    def __init__(
        ...
        *,
        ...
        parsed = urlsplit(url)
        is_local_file = parsed.scheme in ("file", "c", "") or Path(url).exists()
        if not is_local_file and parsed.hostname not in ALLOWED_HOSTS:
            raise ValueError(
                f"Destino {parsed.hostname!r} não permitido para a automação local."
            )
        if is_local_file and not url.startswith("file:"):
            self._url = Path(url).resolve().as_uri()
        else:
            self._url = url
